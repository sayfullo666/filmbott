...
# Admin uchun vaqtinchalik holatlarni saqlash (RAM da)
# Masalan: kod kutyapmizmi, kanal linki kutyapmizmi va h.k.
admin_state = {}      # {admin_id: {"action": "...", ...}}
user_pending_code = {}  # {user_id: "kino_kodi"}  -> obunani tekshirgandan keyin kino yuborish uchun

# Shaxsiy kanalga yangi joylangan, lekin hali botga "kod/nom/tavsif" bilan
# qo'shilmagan videolar shu yerda vaqtincha saqlanadi: {message_id: {...}}
pending_channel_posts = {}

# https://t.me/kanal_username/123  yoki  https://t.me/c/1234567890/123
...
def check_subscription(user_id: int):
    ...
    Qaytaradi: (True/False, obuna_bolmagan_kanallar_royxati)
    """
    ...
    for ch in channels:
        try:
            ...
            # Bot kanalda admin bo'lmasa yoki xatolik bo'lsa, xavfsizlik uchun
...
# ==================== FORWARD QILINGAN XABARLAR (ADMIN) ====================
...
def handle_forward(message):
    ...
    # Agar admin hech qanday amal tanlamagan bo'lsa
    bot.send_message(
        message.chat.id,
...
# ==================== KANALGA AVTOMATIK ULANISH (yangi videolarni aniqlash) ====================
# Bot PERSONAL_CHANNEL_ID kanaliga admin qilib qo'shilgan bo'lsa, kanalga
# joylangan HAR QANDAY video (forward orqali kelgan bo'lsa ham, boshqa
# kanal/guruhdan olingan bo'lsa ham - farqi yo'q) shu handler orqali avtomatik
# aniqlanadi. Bu forward cheklovi (protected content) muammosini butunlay
# chetlab o'tadi, chunki bot videoni o'z kanalidan to'g'ridan-to'g'ri o'qiydi.
...
# ==================== ADMIN MATNLI JAVOBLARI (state asosida) ====================
...
def handle_admin_state_text(message):
    ...
    if action == "waiting_movie_source":
        # Admin forward o'rniga post havolasini yubordi
        ...
        if is_private:
            # https://t.me/c/1234567890/123 -> haqiqiy chat_id = -100 + 1234567890
    ...
    # Boshqa holatlar uchun state ni tozalab yuboramiz
    admin_state.pop(message.from_user.id, None)
